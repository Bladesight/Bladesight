...
def cff_method_single_revolution(
    ...
    tip_deflection_signals = [
        f"x_p{i_probe + 1}{signal_suffix}" for i_probe in range(PROBE_COUNT)
    ]
    ...
    B = A_pinv.dot(df_blade.loc[:, tip_deflection_signals].values.T)
    ...
    df_predicted_targets = pd.DataFrame(
        A.dot(B).T, columns=[col + "_pred" for col in tip_deflection_signals]
    )
...
def cff_method_multiple_revolutions(
    ...
    arr_multiple_thetas = np.array(list(theta_sensors) * (2 * extra_revolutions + 1))
    ...
    X_multiple_revos = np.zeros(
        (new_obs_rows, PROBE_COUNT * (2 * extra_revolutions + 1))
    )
    for n_revo in range(-extra_revolutions, extra_revolutions + 1):
        for i_probe in range(PROBE_COUNT):
            ...
            X_multiple_revos[:, i_col] = df_blade.iloc[mat_aoas_start:mat_aoas_end][
                tip_deflection_signals[i_probe]
            ]
    ...
    predicted_deflections = target_matrix[
        :, extra_revolutions * PROBE_COUNT : (extra_revolutions + 1) * PROBE_COUNT
    ]
    df_predicted_targets = pd.DataFrame(
        predicted_deflections, columns=[col + "_pred" for col in tip_deflection_signals]
    )
    df_cff = pd.concat([df_cff, df_predicted_targets], axis=1)
    return df_cff
...
def perform_CFF_fit(
    ...
    """
    PROBE_COUNT = len([col for col in df_blade.columns if col.endswith("_filt")])
    theta_sensor_set = [
        df_blade[f"AoA_p{i_probe + 1}"].median() for i_probe in range(PROBE_COUNT)
    ]
    errors = []
    df_resonance_window = df_blade.query(f"n >= {n_start} and n <= {n_end}")
    for EO in EOs:
        df_cff_params = cff_method_multiple_revolutions(
            df_resonance_window, theta_sensor_set, EO, extra_revolutions
        )
        error = 0
        for i_probe in range(PROBE_COUNT):
            error += np.sum(
    ...
    return {"df_cff_params": df_cff_params, "EO": EO}
